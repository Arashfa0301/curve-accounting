...
def find_operator(notion, text) -> list:
    print("\n\n\n\na")
    operators = get_database_data(notion, os.getenv("NOTION_MERCHENTS_DB_ID")).get(
        "results"
    )
    print("\n\n\nb")
    print(operators[0]["properties"])
    print("\n\n\nc")

    print(operators[0]["properties"]["Aliaser"])
    ...
    if len(matching_operator_ids) == 0:
        ...
        new_page = notion.pages.create(
            parent={"database_id": os.getenv("NOTION_MERCHENTS_DB_ID")},
            properties={
        ...
        destroy_cache(os.getenv("NOTION_MERCHENTS_DB_ID"))
